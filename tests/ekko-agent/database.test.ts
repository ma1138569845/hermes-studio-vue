...
describe('EkkoDatabaseManager', () => {
  ...
  it('resets the complete legacy skills directory once and reinstalls built-in skills', { timeout: 120000 }, async () => {
  ...
  it.skipIf(process.platform === 'win32')('restores the original database when creating the replacement database fails', async () => {
    const manager = new EkkoDatabaseManager({ baseDirectory: webUiHome })
