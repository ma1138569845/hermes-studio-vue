...
describe('profile agent facade', () => {
  ...
  it('binds tool, runtime, conversation, log, and memory operations to the profile', { timeout: 120000 }, async () => {
    const ekko = new EkkoAgent({
      baseDirectory,
      profiles: ['work'],
      env: { NODE_ENV: 'test' },
  ...
  it('lets a host create a profile runtime before selecting a provider', { timeout: 120000 }, async () => {
  ...
  it('supports process-level profile creation/removal and validates config before construction', { timeout: 120000 }, async () => {
