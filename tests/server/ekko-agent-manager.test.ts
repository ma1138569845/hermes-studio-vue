...
describe('GlobalEkkoAgent', () => {
  ...
  it.skipIf(process.platform === 'win32')('automatically reopens persistent storage after a repaired fallback database', async () => {
  ...
  it('is created once and handles repeated runs through the same runtime', { timeout: 120000 }, async () => {
  ...
  it('lets the runtime own compact model request logs', { timeout: 120000 }, async () => {
    const agent = createTestAgent({ memory: false, profile: 'work' })
    try {
      await agent.run({
