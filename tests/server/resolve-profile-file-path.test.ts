...
const HOME = join(tmpdir(), 'hermes-resolve-home')
...
describe('resolveProfileFilePath', () => {
  ...
  it('still refuses an absolute path', async () => {
    const resolve = await resolveProfileFilePath()
    const absoluteSample = process.platform === 'win32' ? 'C:\\etc\\passwd' : '/etc/passwd'
    expect(() => resolve(absoluteSample)).toThrow(/Invalid file path|Path traversal detected/)
    expect(() => resolve(join(HOME, '..', 'outside', 'skills'))).toThrow(/Invalid file path|Path traversal detected/)
  })
