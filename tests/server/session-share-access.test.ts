...
function symlinkDir(target: string, path: string) {
...
describe('existing APIs with session share credentials', () => {
  ...
  it.skipIf(process.platform === 'win32')('switches only within granted roots and applies relative file paths to the new workspace', async () => {
    const base = join(root, 'workspace'), child = join(base, 'child')
    await mkdir(child); await mkdir(join(base, '.ssh'))
    await symlinkDir(root, join(base, 'escape'))
    await symlinkDir(join(base, '.ssh'), join(base, 'hidden-alias'))
  ...
  it('enforces file actions and both rename paths in the real workspace controllers', async () => {
    ...
    await symlinkDir(root, join(workspace, 'escape'))
  ...
  it('binds upload chunks and downloads to the session without granting workspace access', async () => {
    ...
    await symlinkDir(otherRoot, dirname(result.body.files[0].path))
...
describe('shared speech authorization', () => {
  ...
  it.skipIf(process.platform === 'win32')('aborts pending synthesis when voice permission is revoked', async () => {
    const allowed = await issue({ voice: true })
    let started!: () => void
