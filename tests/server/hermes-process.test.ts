...
describe('Hermes process invocation', () => {
  it('bypasses the uv hermes.exe trampoline on Windows packaged installs', async () => {
    setPlatform('win32')
    process.env.HERMES_BIN = 'C:\\Users\\me\\AppData\\Local\\Programs\\DechnicAuditor\\resources\\python\\Scripts\\hermes.exe'
  ...
  it('defaults spawned Windows Hermes processes to hidden windows', async () => {
    setPlatform('win32')
    process.env.HERMES_BIN = 'C:\\DechnicAuditor\\resources\\python\\Scripts\\hermes.exe'
