...
describe('agent bridge manager command resolution', () => {
  ...
  it('prefers the DechnicAuditor bundled runtime env over a user-installed Hermes command', async () => {
  ...
  it.skipIf(process.platform === 'win32')('uses the Python beside a shell-wrapped hermes command', async () => {
    const binDir = join(tempDir, 'bin')
    const homeDir = join(tempDir, 'home')
  ...
  it('injects Web UI OpenRouter attribution into the bridge process env by default', async () => {
    ...
    expect(env.HERMES_OPENROUTER_APP_TITLE).toBe('DechnicAuditor')
