...
const PATH_MARKER_START = '# >>> DechnicAuditor CLI shim >>>'
const PATH_MARKER_END = '# <<< DechnicAuditor CLI shim <<<'
...
function windowsCliForwarder(runtimePlatform: string, runtimeVersion: string): string {
  ...
  return [
    ...
    "if(!fs.existsSync(python)){console.error('DechnicAuditor Python runtime not found at '+python);console.error('Open DechnicAuditor once to finish runtime setup, then retry hermes-studio cli.');process.exit(127)}",
...
export function createPowerShellShimContent(
  ...
  return [
    ...
    "      [Console]::Error.WriteLine('DechnicAuditor executable not found at ' + $App)",
    '      exit 127',
    '    }',
    ...
    "      [Console]::Error.WriteLine('DechnicAuditor Node runtime not found at ' + $Node)",
    "      [Console]::Error.WriteLine('Open DechnicAuditor once to finish runtime setup, then retry hermes-studio cli.')",
    '      exit 127',
    '    }',
    ...
    '    if (!(Test-Path -LiteralPath $Node -PathType Leaf)) {',
    "      [Console]::Error.WriteLine('DechnicAuditor Node runtime not found at ' + $Node)",
    "      [Console]::Error.WriteLine('Open DechnicAuditor once to finish runtime setup, then retry hermes-studio web.')",
    '      exit 127',
    '    }',
    ...
    '  }',
    '  default {',
    "    [Console]::Error.WriteLine('Unknown DechnicAuditor command: ' + $Command)",
...
export function createShimContent(
  ...
  return [
    ...
    '}',
    'if [ ! -x "$APP" ]; then',
    '  echo "DechnicAuditor executable not found at $APP" >&2',
    '  exit 127',
    'fi',
    ...
    '    shift',
    '    if [ ! -x "$NODE" ]; then',
    '      echo "DechnicAuditor Node runtime not found at $NODE" >&2',
    '      echo "Open DechnicAuditor once to finish runtime setup, then retry hermes-studio web." >&2',
    '      exit 127',
    '    fi',
    ...
    '    ;;',
    '  *)',
    '    echo "Unknown DechnicAuditor command: $1" >&2',
...
export function createMcpShimContent(
  ...
  return [
    ...
    'if [ ! -x "$NODE" ]; then',
    '  echo "DechnicAuditor Node runtime not found at $NODE" >&2',
    '  echo "Open DechnicAuditor once to finish runtime setup, then retry hermes-studio-mcp." >&2',
    '  exit 127',
    'fi',
    'if [ ! -f "$SCRIPT" ]; then',
    '  echo "DechnicAuditor MCP script not found at $SCRIPT" >&2',
    '  exit 127',
    'fi',
...
export function createMcpPowerShellShimContent(
  ...
  return [
    ...
    'if (!(Test-Path -LiteralPath $Node -PathType Leaf)) {',
    "  [Console]::Error.WriteLine('DechnicAuditor Node runtime not found at ' + $Node)",
    "  [Console]::Error.WriteLine('Open DechnicAuditor once to finish runtime setup, then retry hermes-studio-mcp.')",
    '  exit 127',
    '}',
    'if (!(Test-Path -LiteralPath $Script -PathType Leaf)) {',
    "  [Console]::Error.WriteLine('DechnicAuditor MCP script not found at ' + $Script)",
    '  exit 127',
    '}',
...
export async function installHermesStudioCliShim(options: CliShimInstallOptions = {}): Promise<CliShimInstallResult> {
  ...
  return {
    ...
    status,
    pathUpdated,
    reason: status === 'skipped' ? 'existing hermes-studio shim is not managed by DechnicAuditor' : undefined,
  }
}
...
export async function installHermesStudioMcpShim(options: McpShimInstallOptions = {}): Promise<CliShimInstallResult> {
  ...
  return {
    ...
    status,
    pathUpdated,
    reason: status === 'skipped' ? 'existing hermes-studio-mcp shim is not managed by DechnicAuditor' : undefined,
  }
}
